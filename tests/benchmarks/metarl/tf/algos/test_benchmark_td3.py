"""
This script creates a regression test over metarl-TD3.

It get Mujoco1M benchmarks from baselines benchmark, and test each task in
its trail times on metarl model and rlkit model. For each task, there will
be `trail` times with different random seeds. For each trail, there will be two
log directories corresponding to rlkit and metarl. And there will be a plot
plotting the average return curve from rlkit and metarl.
"""
import datetime
import os
...
from metarl.np.exploration_strategies.gaussian_strategy import GaussianStrategy
...
# Hyperparams for rlkit and metarl
params = {
    'policy_lr': 1e-3,
    'qf_lr': 1e-3,
...
class TestBenchmarkTD3:
    """Benchmark MetaRL TD3 implementation with rlkit's implementation."""

    @pytest.mark.huge
    def test_benchmark_td3(self):
        """
        Test metarl TD3 benchmarks.
