...
ci-verify-conda: METARL_BIN = $(CONDA_ROOT)/envs/metarl-ci/bin
ci-verify-conda:
...
ci-deploy-docker:
	echo "${DOCKER_API_KEY}" | docker login -u "${DOCKER_USERNAME}" \
		--password-stdin
	docker tag "${TAG}" icml2020submission6857/metarl-ci:latest
	docker push icml2020submission6857/metarl-ci

build-ci: TAG ?= icml2020submission6857/metarl-ci:latest
...
build-headless: TAG ?= icml2020submission6857/metarl-headless:latest
...
build-nvidia: TAG ?= icml2020submission6857/metarl-nvidia:latest
...
run-ci: TAG ?= icml2020submission6857/metarl-ci
run-ci:
	docker run \
		-e TRAVIS_BRANCH \
...
run-headless: build-headless
	docker run \
		...
		--name $(CONTAINER_NAME) \
		${ADD_ARGS} \
		icml2020submission6857/metarl-headless $(RUN_CMD)
...
run-nvidia: build-nvidia
	...
	docker run \
		...
		--name $(CONTAINER_NAME) \
		${ADD_ARGS} \
		icml2020submission6857/metarl-nvidia $(RUN_CMD)
