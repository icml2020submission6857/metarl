...
algorithm provided in the algorithm, available at :code:`metarl/tf/algos/vpg.py`. It utilizes
...
- Vanilla Policy Gradient: :code:`metarl/tf/algos/vpg.py`

- Natural Policy Gradient: :code:`metarl/tf/algos/npo.py`

- Truncated Natural Policy Gradient: :code:`metarl/tf/algos/tnpg.py`

- Reward-Weighted Regression: :code:`metarl/tf/algos/erwr.py`

- Trust Region Policy Optimization: :code:`metarl/tf/algos/trpo.py`

- Proximal Policy Optimization: :code:`metarl/tf/algos/ppo.py`

- Relative Entropy Policy Search: :code:`metarl/tf/algos/reps.py`

To give an illustration, here's how we might implement :code:`init_opt` for VPG
(the actual code in :code:`metarl/tf/algos/vpg.py` is longer due to the need to log
extra diagnostic information as well as supporting recurrent policies):

.. code-block:: python

    from metarl.misc.ext import extract, compile_function, new_tensor
...
we use an optimizer, which in this case would be an instance of :code:`metarl.optimizers.first_order_optimizer.FirstOrderOptimizer`.
...
The :code:`metarl.parallel_sampler` module takes care of parallelizing the
